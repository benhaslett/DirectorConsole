...
**Description:** An end-to-end workflow manager for AI music videos and short films. It orchestrates concept development, shot listing, image generation (ComfyUI), and video animation (LTX.2).
...
# Animate Video Clips (LTX.2)
node skills/director/scripts/shoot.js "Project Name" --phase video
```

### 4. Review (Dailies)
Check status and list missing assets.

```bash
node skills/director/scripts/status.js "Project Name"
```

### 5. Director's Console (GUI)
...
- Requires `comfy-art` workflows (Qwen & LTX.2).
